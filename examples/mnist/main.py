...
class Hardshrink(nn.Module):
    ...
    def forward(self, x):
        lambd = torch.tensor(self.lambd, device=x.device, dtype=x.dtype)
        zero = torch.tensor(0.0, device=x.device, dtype=x.dtype)
        return torch.where(torch.logical_or(x > lambd, x < -lambd), x, zero)

class Net(nn.Module):
    def __init__(self):
        super(Net, self).__init__()
        self.conv1 = nn.Conv2d(1, 32, 3, 1)
        self.conv1_act = Hardshrink(0.5)
        self.conv2 = nn.Conv2d(32, 64, 3, 1)
        self.conv2_act = Hardshrink(0.5)
        self.dropout1 = nn.Dropout(0.25)
        self.dropout2 = nn.Dropout(0.5)
        self.fc1 = nn.Linear(9216, 128)
        self.fc1_act = Hardshrink(0.5)
